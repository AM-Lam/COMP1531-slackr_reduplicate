...
def test_message_send():
    ...
    user2 = {
        "token" : jwt.encode({"u_id" : "112"}, secret, algorithm="HS256").decode(),
        "u_id" : "112"
    }

    user3 = {
        "token" : jwt.encode({"u_id" : "113"}, secret, algorithm="HS256").decode(),
        "u_id" : "113"
    }
    ...
    pytest.raises(ValueError, message_send, user3["token"], 1,
                "Hello")

    # reset message_1
    message_1 = None
    # the message is over 1000 characters
    pytest.raises(ValueError, message_send, user1["token"], 111,
