...
def message_react(token, message_id, react_id):
    ...
    if message_id not in message_id_list:
        raise ValueError(description="The message no longer exists.")
    ...
    if id_of_channel not in channels_list(token_of_message):
        raise ValueError(description="You are not a member of the channel.")
    ...
    if react_id_dic[message_id][2] != None:
        return ValueError(description="You reacted before. Please don't repeat.")
