import pytest
import message_(un)sendOredit
#TODO
# have to import from other function
from channel_leave.file import channel_leave

# Setup
token = setup.generateToken('hayden@gmail.com', '123456', 'Hayden', 'Smith')
channel_id = setup.generateChannel(token, "Channel 1", True)
messageId = "456789"

def test_message_send(token, channel_id):
    assert message_send(token, channel_id, 'hello') == None
    #case with 1000 characters
    assert message_send(token, channel_id, '') == None
    assert message_send(token, channel_id, '    ') == None


    # Message is more than 1000 characters
    with pytest.raises(ValueError):
        message_send(token, channel_id, '')
        message_send(token, channel_id, '
')


    
    
def test_message_remove(token, message_id):
    assert message_remove(token, message_id) == None
    # sender remove the message 
    assert message_remove('123', 'message_id') == None
    #admin remove the message
    assert message_remove('456', 'message_id') == None

    # ValueError 
    # Message is not existed
    with pytest.raises(ValueError):
        message_remove('123', '_id')
        message_remove('123', 'mesage')

    # Message (based on ID) no longer exists
    def test_message_no_longer_exist:
        # remove the message first
        message_remove('123', 'message_id')
        message_remove('AD', 'message_id')
        # Now the message is no longer exists
        with pytest.raises(ValueError):
            message_remove('123', 'message_id')

    # AccessError: 
        # if user is not a member of a certain channel anymore
        def test_error_leave_channel:
            channel_leave(token, 'Channel 1')
            with pytest.raises(AccessError) :
                message_remove('123', 'id')

    with pytest.raises(AccessError): 
        # Admin of channel A is not channel B but they try to delete message in channel B
        message_remove('456', 'message_id')
        message_remove('456', 'id')


def test_message_edit(token, message_id, message):
    assert message_edit('123', 'message_id', 'hello') == None
    assert message_edit('123', 'message_id', 'he ll.;...,o JHBF65154') == None
    assert message_edit('123', 'message_id', 'KJENF39482UEhUr8u309rqj') == None
    assert message_edit('123', 'message_id', 'GKREgW2twKzcZ9wMe3zMsl0yf8OJfYOAB9I9DWd81pIPRmlq7v1p5j1tfifWu8kcoXkG1wu0qLK7xfhHlJUg12Yxr') == None

    with pytest.raises(ValueError):
    # ValueError when:
    #  Message with message_id edited by authorised user is not the poster of the message
        message_edit('789', 'message_id', 'hello')
        message_edit('123', 'message', 'hello')

    #  Message with message_id is not a valid message
    #     1) is a message sent by the authorised user
    # The message is over 1000 characters
        message_edit('123', 'message_id', '')
    #     2) If the authorised user is an admin, is a any message within a channel that the authorised user has joined
        message_edit('789', 'message_id', 'hello')
        message_edit('789', 'id', 'hello')
