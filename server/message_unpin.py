...
def message_unpin(token, message_id):
    ...
    if message_id not in message_id_list:
        raise ValueError(description="Message is not exists.")

    #  The authorised user is not an admin
    if id_of_channel not in channels_list(token_of_message):
        raise ValueError(description="Sorry, you don't have access to unpin the message.")
    
    #  Message with ID message_id is already unpinned
    if token not in pinned_list[message_id]:
        raise ValueError(description="The message is not pinned. Please try again.")
    ...
    if id_of_channel not in channels_list(token_of_message):
        raise AccessError(description="You are not a member in the channel.")
